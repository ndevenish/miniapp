...
__device__ KernelThreadParams calculate_block_info(cg::thread_block block, int radius) {
    ...
    threadParams.x = block.group_index().x * block.group_dim().x + block.thread_index().x;
    threadParams.y = block.group_index().y * block.group_dim().y + block.thread_index().y;
 ...
 */
__device__ void load_central_pixels(cg::thread_block block, const KernelThreadParams &threadParams, uint8_t *mask, uint8_t *shared_mask, 
                                    size_t mask_pitch, int width, int height, int radius) {
    // Load central pixels into shared memory
    if (threadParams.x < width && threadParams.y < height) {
        shared_mask[threadParams.local_y * threadParams.shared_width + threadParams.local_x] = mask[threadParams.y * mask_pitch + threadParams.x];
    } else {
        shared_mask[threadParams.local_y * threadParams.shared_width + threadParams.local_x] = MASKED_PIXEL;
    }
}
 ...
 * @param radius The radius around each masked pixel to be considered. 
 */
__device__ void load_border_pixels(cg::thread_block block, const KernelThreadParams &threadParams, uint8_t *mask, uint8_t *shared_mask, 
                                   size_t mask_pitch, int width, int height, int radius) {
    // Load border pixels into shared memory
    for (int i = block.thread_index().x; i < threadParams.shared_width; i += block.group_dim().x) {
        for (int j = block.thread_index().y; j < threadParams.shared_height; j += block.group_dim().y) {
            int global_x = threadParams.x + (i - threadParams.local_x);
            int global_y = threadParams.y + (j - threadParams.local_y);

            bool is_within_central_region = (i >= radius && i < threadParams.shared_width - radius && j >= radius && j < threadParams.shared_height - radius);
            ...
            if (is_global_x_in_bounds && is_global_y_in_bounds) {
                shared_mask[j * threadParams.shared_width + i] = mask[global_y * mask_pitch + global_x];
            } else {
 ...
 */
__device__ bool determine_erasure(const KernelThreadParams &threadParams, const uint8_t *shared_mask, int radius, int distance_threshold) {
    bool should_erase = false;
    for (int i = -radius; i <= radius; ++i) {
        for (int j = -radius; j <= radius; ++j) {
            if (shared_mask[(threadParams.local_y + j) * threadParams.shared_width + (threadParams.local_x + i)] == MASKED_PIXEL) {
 ...
 * @return True if the current pixel should be erased, false otherwise.
 */
__device__ bool launch_erasure_kernel(const uint8_t *shared_mask, const KernelThreadParams &threadParams, int radius, int distance_threshold) {
    ...
    determine_erasure_kernel<<<1, erasure_block_size>>>(shared_mask, threadParams.shared_width, threadParams.local_x, threadParams.local_y, radius, distance_threshold, d_should_erase);
...
__global__ void erosion_kernel(uint8_t *mask,
    ...
    // Load central pixels
    load_central_pixels(block, threadParams, mask, shared_mask, mask_pitch, width, height, radius);

    // Load border pixels
    load_border_pixels(block, threadParams, mask, shared_mask, mask_pitch, width, height, radius);
    ...
    */
    ...
    // for-loop based
    bool should_erase = determine_erasure(threadParams, shared_mask, radius, 2); // Use 2 as the Chebyshev distance threshold
    // dynamic parrelism based
    bool should_erase = launch_erasure_kernel(shared_mask, threadParams, radius, 2); // Use 2 as the Chebyshev distance threshold
