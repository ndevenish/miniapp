...
double time_parallelism_over_images(h5read_handle* obj,
                                    int n_images,
                                    void** spotfinders,
                                    int* full_results);
double time_parallelism_over_images_using_modules(h5read_handle* obj,
                                                  int n_images,
                                                  int n_modules,
                                                  void** mini_spotfinders,
                                                  int* full_results_m);
double time_parallelism_over_modules(h5read_handle* obj,
                                     ...
                                     void** mini_spotfinders,
                                     int* mini_results);
double time_parallelism_over_modules_using_floats(h5read_handle* obj,
                                                  int n_images,
                                                  int n_modules,
                                                  void** mini_spotfinders_f,
                                                  int* mini_results_f);
double time_parallelism_over_both(h5read_handle* obj,
                                  int n_images,
                                  int n_modules,
                                  void** mini_spotfinders,
                                  int* both_results,
                                  int n_outer);
double time_parallelism_over_images_using_modules_noblit(h5read_handle* obj,
                                                         int n_images,
                                                         void** noblit_spotfinders,
                                                         int* full_results_m_nb);
double time_parallelism_over_both_noblit(h5read_handle* obj,
                                         int n_images,
                                         void** noblit_spotfinders,
                                         int* both_results_nb,
                                         int n_outer);
...
int main(int argc, char** argv) {
    ...
    double over_both_noblit_time =
      time_parallelism_over_both_noblit(obj,
                                        ...
                                        noblit_spotfinders,
                                        both_results_nb,
                                        2);  // Hard to change outer num properly
    ...
    for (size_t j = 0; j < 5; j++) {
        ...
        } else if (full_results[j] == mini_results[j]
                   && full_results[j] == full_results_m[j]) {
            col = "\033[32m";
        }
...
double time_parallelism_over_images(h5read_handle* obj,
    ...
    image_t* image;
    double t0 = omp_get_wtime();
#pragma omp parallel for default(none) private(image, temp) \
  shared(n_images, obj, spotfinders, full_results)
    for (size_t j = 0; j < n_images; j++) {
        image = h5read_get_image(obj, j);
        temp = spotfinder_standard_dispersion(spotfinders[omp_get_thread_num()], image);
        h5read_free_image(image);
...
double time_parallelism_over_images_using_modules(h5read_handle* obj,
    ...
    image_modules_t* modules;
    double t0 = omp_get_wtime();
#pragma omp parallel for default(none) private(modules, strong_pixels_from_modules) \
  shared(n_images, n_modules, obj, mini_spotfinders, full_results_m)
    for (size_t j = 0; j < n_images; j++) {
        modules = h5read_get_image_modules(obj, j);
...
#pragma omp parallel for default(none) private(modules, strong_pixels_from_modules) \
  shared(n_images, n_modules, obj, mini_spotfinders, both_results, outer_num)       \
...
#pragma omp parallel for default(none) shared(modules, mini_spotfinders, n_modules,offset) reduction(+:strong_pixels_from_modules) num_threads(omp_get_max_threads()/outer_num)
        for (size_t n = 0; n < n_modules; n++) {
            strong_pixels_from_modules += spotfinder_standard_dispersion_modules(
              mini_spotfinders[offset + omp_get_thread_num()], modules, n);
        }
        h5read_free_image_modules(modules);
...
#pragma omp parallel for default(none) private(result, image) \
  shared(obj, noblit_spotfinders, full_results_m_nb, n_images)
    for (size_t j = 0; j < n_images; j++) {
        image = h5read_get_image(obj, j);
        result = spotfinder_standard_dispersion_modules_new(
          noblit_spotfinders[omp_get_thread_num()], image);
...
int module_to_image_index(int module_num, int module_idx) {
    ...
    size_t offset = r_0 + (module_idx / E2XE_MOD_FAST) * E2XE_16M_FAST
                    + i_fast * (E2XE_MOD_FAST + E2XE_GAP_FAST);
