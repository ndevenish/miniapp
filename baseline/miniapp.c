...
double time_baselines(h5read_handle* obj,
    ...
    if (preload) {
        ...
        for (size_t j = 0; j < n_images_to_use; j++) {
            image = h5read_get_image(obj, j);
            temp =
              spotfinder_standard_dispersion(spotfinders[omp_get_thread_num()], image);
            h5read_free_image(image);
            full_results[j] = temp;
        }
        return ((double)n_images / (double)n_images_to_use) * (omp_get_wtime() - t0);
...
double time_parallelism_over_modules_noblit(h5read_handle* obj,
                                            int n_images,
    ...
    if (preload) {
        ...
        for (size_t j = 0; j < n_images; j++) {
            result = spotfinder_standard_dispersion_modules_new(
              noblit_spotfinders[omp_get_thread_num()], images[j]);
