...
image_t_type _read_single_value(hid_t dataset, const char* dataset_name) {
  ...
  if (num_elements > 1) {
    fprintf(stderr, "Error: While reading %s: More than one element.\n",
            dataset_name);
    exit(1);
  }
  if (datatype_size < sizeof(image_t_type)) {
    fprintf(stderr,
            "Error: While reading %s: Value of size %zu is smaller than data "
            "type\n",
            dataset_name, datatype_size);
    exit(1);
  }
  assert(sizeof(image_t_type) == 2);
  image_t_type value;
  if (H5Dread(dataset, H5T_NATIVE_UINT16, H5S_ALL, H5S_ALL, H5P_DEFAULT,
              &value) < 0) {
    fprintf(stderr,
            "Error: While reading %s: Unspecified data reading error.\n");
    exit(1);
  }
  return value;
}
...
void read_trusted_range(h5read_handle* obj) {
  // char mask_path[] = ;
  hid_t saturation_dataset =
      H5Dopen(obj->master_file, "/entry/instrument/detector/saturation_value",
              H5P_DEFAULT);
  hid_t underload_dataset =
      H5Dopen(obj->master_file, "/entry/instrument/detector/underload_value",
              H5P_DEFAULT);
...
void read_wavelength(h5read_handle* obj) {
  hid_t dataset =
      H5Dopen(obj->master_file, "/entry/instrument/beam/incident_wavelength",
              H5P_DEFAULT);
  if (dataset < 0) {
